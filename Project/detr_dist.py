...
Original file is located at
    https://colab.research.google.com/drive/1Yc7qm6FoURXUhouwNvhvs8rUXpf_wfhV
"""
...
class SaveOutput:
    ...
    def __call__(self, layer, module_in, module_out):
        # print(module)
        # self.inputs.append(module_in)
        self.outputs[layer.__name__] = module_out
        # print("!")
...
model_teacher = torch.hub.load("facebookresearch/detr", "detr_resnet101", pretrained = False).cuda()
# module_for_loader = torch.hub.load("facebookresearch/detr", "detr_resnet101", pretrained = False)
model_teacher.class_embed = nn.Linear(in_features = 256, out_features = len(CLASSES)+1, bias = True)
model_teacher.load_state_dict(torch.load("/content/drive/MyDrive/net_params.pth"))
model_teacher.to(device)

model_student = copy.deepcopy(model_teacher)

model_student.transformer.encoder.layers = copy.deepcopy(model_teacher.transformer.encoder.layers[-3:-1])
model_student.transformer.decoder.layers = copy.deepcopy(model_teacher.transformer.decoder.layers[-3:-1])
model_student.eval()

model_student.load_state_dict(torch.load("/content/drive/MyDrive/net_params_stu.pth"))
target_dic = get_tatget_dic()

hook_handles = []
# inter_layers = ['transformer.encoder', 'transformer.decoder', 'class_embed', 'bbox_embed', 'transformer.decoder.layers.5.norm3']
inter_layers_teacher = ['transformer.decoder.layers.2', 'transformer.encoder.layers.2', 'transformer.decoder.layers.5', 'transformer.encoder.layers.5']
inter_layers_student = ['transformer.decoder.layers.0', 'transformer.encoder.layers.0', 'transformer.decoder.layers.1', 'transformer.encoder.layers.1']

teacher_output = SaveOutput()
student_output = SaveOutput()

for name, layer in model_teacher.named_modules():
    layer.__name__ = name
    # print(name)
    if name in inter_layers_teacher:
      handle = layer.register_forward_hook(teacher_output)
      hook_handles.append(handle)

for name, layer in model_student.named_modules():
    layer.__name__ = name
    if name in inter_layers_student:
      handle = layer.register_forward_hook(student_output)
      hook_handles.append(handle)
# img_stu = copy.deepcopy(img)
...
optimizer = optim.AdamW([{'params': model_student.parameters(), 'lr': learning_rate}])
soft_criterion = nn.MSELoss()
hu = HungarianMatcher()
hard_criterion = SetCriterion(num_classes, hu, {'loss_ce': 1, 'loss_bbox': 1, 'loss_giou': 3}, 0.1, ['labels', 'boxes', 'cardinality'])
...
for epoch in range(1, num_epochs + 1):
    ...
    for i in range(int(900/batch_size)):
        ...
        out_te = model_teacher(data)
        for item in out_te:
          out_te[item].detach
        for item in teacher_output.outputs:
          teacher_output.outputs[item].detach()

        out_stu = model_student(data)

        soft_loss_encoder_1 = soft_criterion(student_output.outputs['transformer.encoder.layers.1'], teacher_output.outputs['transformer.encoder.layers.5'])
        soft_loss_decoder_1 = soft_criterion(student_output.outputs['transformer.decoder.layers.1'], teacher_output.outputs['transformer.decoder.layers.5'])
        soft_loss_encoder_0 = soft_criterion(student_output.outputs['transformer.encoder.layers.0'], teacher_output.outputs['transformer.encoder.layers.2'])
        soft_loss_decoder_0 = soft_criterion(student_output.outputs['transformer.decoder.layers.0'], teacher_output.outputs['transformer.decoder.layers.2'])
        soft_loss_class = soft_criterion(out_te['pred_boxes'], out_stu['pred_boxes'])
        soft_loss_bbox = soft_criterion(out_te['pred_logits'], out_stu['pred_logits'])

        softloss = (soft_loss_encoder_0 + soft_loss_decoder_0 + soft_loss_encoder_1 + soft_loss_decoder_1)*0.5 + soft_loss_class + soft_loss_bbox

        loss_dict = hard_criterion(out_stu, target)
        weight_dict = hard_criterion.weight_dict
        hard_loss = sum(loss_dict[k] * weight_dict[k] for k in loss_dict.keys() if k in weight_dict)

        loss = alpha*softloss + (1-alpha)*hard_loss
        loss = softloss
        if i%50 == 0:
        ...
        loss.backward()
        optimizer.step()
# soft_loss_encoder = soft_criterion(F.log_softmax(student_output.outputs['transformer.encoder.layers.1']/temp,dim=1),F.softmax(teacher_output.outputs['transformer.encoder.layers.5']/temp,dim=1))*temp*temp
# soft_loss_decoder = soft_criterion(F.log_softmax(student_output.outputs['transformer.decoder.layers.1']/temp,dim=1),F.softmax(teacher_output.outputs['transformer.decoder.layers.5']/temp,dim=1))*temp*temp


# loss.backward()
# hard_loss = hard_criterion(out_stu,target)
# soft_loss_encoder.backward()

compute_ap(out_stu, target, hu)

# torch.save(model_student.state_dict(), '/content/drive/MyDrive/net_params_stu.pth')
...
ref_and_show(im, model_teacher, threshold=[0.12])
...
teacher_output.outputs['bbox_embed'][0,:,:,:] - teacher_output.outputs['bbox_embed'][1,:,:,:]

student_output.outputs['bbox_embed'].shape

student_output.outputs['transformer.decoder.layers.1.norm3'].shape
